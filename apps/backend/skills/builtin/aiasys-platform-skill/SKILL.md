...
会话是工作区内的对话上下文。同一个工作区可以开多个会话，每个会话有独立的对话历史和执行状态，但共享工作区的文件和 Skill。
...
2. **系统 Skill**：依赖 AIASys 内部服务或内置对象模型，如本 skill（平台操作）、competition-research-skill（单会话竞赛实验）、competition-runtime-prep-skill（竞赛环境准备）、competition-parallel-research-skill（竞赛并行编排）、aiasys-canvas-skill（Canvas 对象编辑）
...
Session 预算由当前会话的预算控制统一管理，不是 Auto Task 参数。预算耗尽后运行时会阻止该会话继续执行；如果 continuous Auto Task 绑定了这条会话，系统会同步暂停该任务。
...
5. Agent 每轮按 `competition-research-skill` 的单会话循环执行：preflight、跑实验、记录结果、更新研究视图
6. 实验完成后 Agent 更新 `experiments/index.json`
7. 系统检查 `auto_task_signal`，决定是否进入下一轮

多会话、多环境并行探索不直接塞进单条 continuous Auto Task。用户明确授权并行时，先用 `competition-parallel-research-skill` 规划 lane、`env_id`、session、AutoTask 和写回顺序。
...
competition-research-skill → 摄入知识图谱
    ↓
competition-research-skill → 单会话 preflight + 实验规划
    ↓
