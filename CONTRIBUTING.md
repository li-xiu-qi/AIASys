...
- **流程高于便利**：默认情况下，所有改动（包括维护者/管理员自己的改动）都必须走 PR 流程，禁止直接 push `main` 或 `dev`。规范是给所有人遵守的，不是用来约束外部贡献者的。特殊情况下（如批量同步、紧急 hotfix、已有 PR 的后续修正）可直接推送，但必须在 PR 或 commit message 中说明例外原因，并尽快补齐 PR 记录。
...
4. **协调优先**：若 Issue 已有评论认领且认领者正在推进，后续贡献者应优先在 Issue 下留言表明愿意协助或 review，而非重复认领或直接另提 PR。维护者可介入协调分工或合并推进。
...
- **默认任何改动都不要直接 push 到 `main` 或 `dev`，必须走 PR 流程**。维护者/管理员自己的改动也不例外，至少在 GitHub 上留下 PR 记录和 review 痕迹。特殊情况下（如批量同步、紧急 hotfix、已有 PR 的后续修正）可直接推送，但必须在 PR 或 commit message 中说明例外原因，并确保变更可追溯。
