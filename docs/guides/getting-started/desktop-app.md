...
为兼顾不同网络环境和用户偏好，桌面版提供三种安装变体：

| 变体 | 构建命令 | 体积 | 首次启动 | 说明 |
|------|----------|------|----------|------|
| **Full** | `npm run dist:win` | ~250MB | 解压即用 | 内置完整 Python 3.12 运行时，离线可用 |
| **Light** | `npm run dist:win:light` | ~50MB | 首次启动自动下载 Python | 不内置 Python，首次启动时用 bundled uv 下载并创建环境 |
| **Portable** | `npm run dist:win:portable` | ~50MB | 首次启动自动下载 Python | zip 免安装版，解压即用 |

Full 模式适合离线环境或网络不稳定的用户；Light/Portable 模式体积小 80%，适合网络良好的用户。

**首次启动流程（Light/Portable 模式）**：
```
启动应用 → 检测到无 Python 环境
  → uv python install 3.12（下载 python-build-standalone，约 30MB）
  → uv venv（创建虚拟环境）
  → uv sync（安装 Python 依赖，约 200 个包）
  → 启动后端服务
```
整个过程约 2-3 分钟，后续启动秒开。进度通过启动画面实时展示。
...
- `prepare-runtime.cjs` 自动清理 `__pycache__` 和 `.pyc` 文件，剔除开发依赖减小体积
- Full 模式：`.venv` 压缩为 `.venv.tar.gz`（体积减少 60-70%），首次启动时解压到运行时目录
- Light/Portable 模式：不内置 Python 运行时，首次启动时用 bundled uv 从零创建环境
