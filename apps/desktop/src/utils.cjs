...
function fixPyvenvHomeIfNeeded(backendRoot) {
  ...
  try {
    ...
    console.log(`[aiasys] 已修复 pyvenv.cfg home 路径: ${expectedHome}`);
  } catch (error) {
    console.warn("[aiasys] 修复 pyvenv.cfg 失败:", error);
  }
  ...
  if (fs.existsSync(venvBinDir) && fs.existsSync(embedBinDir)) {
    for (const name of ["python", "python3"]) {
      ...
      console.log(`[aiasys] 已修复 venv 符号链接: ${name} -> ${relativeTarget}`);
    }
  }
}
...
async function resolveDesiredPort({
  ...
  if (inspection.reason === "not_running") {
    ...
    console.warn(
      `[aiasys] ${label} 端口 ${requestedPort} 复核发现已被占用，自动切换到 ${fallbackPort}`,
    );
    return {
      port: fallbackPort,
      reuse: false,
  ...
  if (inspection.reason === "occupied_current") {
    ...
    if (pid) {
      console.warn(
        `[aiasys] ${label} 端口 ${requestedPort} 上存在当前 checkout 的异常进程，尝试终止: ${processCommand}`,
      );
  ...
  console.warn(
    `[aiasys] ${label} 端口 ${requestedPort} 不可直接复用，自动切换到 ${fallbackPort}: ${processCommand}`,
  );
  return {
    port: fallbackPort,
    reuse: false,
...
function terminateProcessSync(pid, graceMs = 1500) {
  ...
  try {
    ...
    console.warn(`[aiasys] 终止进程 ${pid} 失败: ${error.message}`);
  }
}
...
function terminateProcessTreeSync(pid, graceMs = 1500) {
  ...
  if (process.platform === "win32") {
    try {
      ...
      console.warn(`[aiasys] 终止进程树 ${pid} 失败: ${error.message}`);
    }
    return;
  }
