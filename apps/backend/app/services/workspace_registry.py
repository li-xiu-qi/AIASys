...
class WorkspaceRegistryService:
    ...
    def _write_json(self, path: Path, payload: Any) -> None:
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as f:
                f.write(data)
            # Windows 上 os.replace 可能因杀毒软件扫描临时文件而失败（PermissionError）
            # 重试最多 3 次，每次间隔 0.1s
            sys_temp_path = as_system_path(temp_path)
            sys_target_path = as_system_path(path)
            for attempt in range(3):
                try:
                    os.replace(sys_temp_path, sys_target_path)
                    break
    ...
    def _initialize_workspace_resources(
        ...
        logger.info("工作区 %s 初始化线程启动", workspace_id)
        ...
        try:
            ...
            logger.exception("工作区 %s 更新初始化状态为 running 失败", workspace_id)
            # 继续执行，即使状态更新失败也不影响初始化
