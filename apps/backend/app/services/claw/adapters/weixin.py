...
from app.services.claw.adapters.helpers import MessageDeduplicator  # noqa: E402
from app.services.claw.adapters.models import Platform, PlatformConfig  # noqa: E402
from app.services.claw.adapters.utils import atomic_json_write  # noqa: E402
...
class ContextTokenStore:
    ...
    def restore(self, account_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                "weixin: failed to restore context tokens for %s: %s", _safe_id(account_id), exc
            )
            return
        restored = 0
        ...
        if restored:
            logger.info(
                "weixin: restored %d context token(s) for %s", restored, _safe_id(account_id)
            )
    ...
    def _persist(self, account_id: str) -> None:
        ...
        try:
            ...
            logger.warning(
                "weixin: failed to persist context tokens for %s: %s", _safe_id(account_id), exc
            )
...
def _cdn_download_url(cdn_base_url: str, encrypted_query_param: str) -> str:
    return f"{cdn_base_url.rstrip('/')}/download?encrypted_query_param={quote(encrypted_query_param, safe='')}"  # noqa: E501
...
def _normalize_markdown_blocks(content: str) -> str:
    ...
    while i < len(lines):
        ...
        if i + 1 < len(lines) and "|" in lines[i] and _TABLE_RULE_RE.match(lines[i + 1].rstrip()):
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._cdn_base_url = (
            str(extra.get("cdn_base_url") or os.getenv("WEIXIN_CDN_BASE_URL", WEIXIN_CDN_BASE_URL))
            .strip()
            .rstrip("/")
        )
        ...
        self._dm_policy = (
            str(extra.get("dm_policy") or os.getenv("WEIXIN_DM_POLICY", "open")).strip().lower()
        )
        self._group_policy = (
            str(extra.get("group_policy") or os.getenv("WEIXIN_GROUP_POLICY", "disabled"))
            .strip()
            .lower()
        )
    ...
    async def send_voice(
        ...
        return await self.send_document(
            chat_id, audio_path, caption=caption or "", metadata=metadata
        )
...
async def send_weixin_direct(
    *,
    ...
    if not account_id:
        return {
            "error": "Weixin account ID missing. Configure WEIXIN_ACCOUNT_ID or platforms.weixin.extra.account_id."  # noqa: E501
        }
