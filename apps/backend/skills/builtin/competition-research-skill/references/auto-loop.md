...
本 skill 的自动循环默认是单会话、单环境、串行实验闭环。运行环境必须先由工作区、当前会话或 `competition-runtime-prep-skill` 准备好。自动循环可以做轻量预检，但不能把安装大依赖、切换 GPU 框架或创建多个并行 lane 混入正式实验轮。
...
- 如果发现并行 Agent 或另一个 AutoTask 会话正在同一工作区运行实验，本轮必须暂停或只做记录整理，不要再启动新 runner。
