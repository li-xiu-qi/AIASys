    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionResetPolicy":
        ...
        return cls(
            ...
            notify_exclude_platforms=(
                tuple(exclude) if exclude is not None else ("api_server", "webhook")
            ),
        )
...
class GatewayConfig:
    ...
    group_sessions_per_user: bool = (
        True  # Isolate group/channel sessions per participant when user IDs are available
    )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        if stt_enabled is None:
            stt_enabled = (
                data.get("stt", {}).get("enabled") if isinstance(data.get("stt"), dict) else None
            )
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    if api_server_enabled or api_server_key:
        ...
        if api_server_cors_origins:
            origins = [
                origin.strip() for origin in api_server_cors_origins.split(",") if origin.strip()
            ]
            if origins:
        ...
        config.platforms[Platform.BLUEBUBBLES].extra.update(
            {
                ...
                "send_read_receipts": os.getenv("BLUEBUBBLES_SEND_READ_RECEIPTS", "true").lower()
                in ("true", "1", "yes"),
            }
        )
