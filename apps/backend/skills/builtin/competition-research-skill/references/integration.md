...
| 多会话、多环境并行探索编排 | `competition-parallel-research-skill` |
| Shell 执行 | Shell |
| 工作区环境变量 | 环境变量工具 |
| 当前工作区资源核验 | 资源验证工具 |
...
创建 continuous Auto Task 时优先使用 `overlap_policy=skip`。同一工作区只允许一个自动研究会话启动 runner；如果确实需要排队，使用 `queue`，不要默认并行。手动“立即运行”和后台轮询必须共用同一任务锁，避免同时创建两个自动任务会话。

停止语义要写清：

- 暂停 Auto Task：阻止后续触发。
- 停止当前会话执行：终止正在运行的 Agent 会话。
...
竞赛项目一般需要 Python 环境。单会话实验 AutoTask 默认只使用已经准备好或已经绑定的环境。
...
本 skill 默认不启动多个并行实验会话。需要同时探索多条路线时，先转交 `competition-parallel-research-skill`。
