...
更多演示用例在内部路径 `design-draft/agent-tests/l2-user-scenario-tests/cases/` 维护。这里按真实任务记录数据、推荐提示词、验收标准和运行指标。当前已初始化的重点用例包括知识库问答、知识图谱关系探索、Canvas 工作流画布、子 Agent 协作、AutoTask 触发监控和 PDF 翻译 Skill。README 只放已经跑通并有截图证据的素材，空状态截图和旧烟测截图不再作为对外展示图。
