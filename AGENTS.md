...
> 如果你是在 Windows 侧查看本项目，或需要快速了解本项目有哪些 Skill 可用，
> 请先阅读 `aiasys-skill-index.md`（单个文档索引）。
>
> 生成方式：`bash resources/xiaoke-skill-development/skills/_global/wsl-windows-bridge/scripts/index-wsl-project-skills.sh /home/ke/projects/AIASys`
...
只改 `main.cjs`/`preload.cjs` 时不需要完整打包，直接用 `dev:patch` 增量构建，3 秒完成：

```bash
# WSL 内执行，自动同步源码到 Windows 临时目录，检测变动范围，只构建受影响部分
cd apps/desktop && npm run dev:patch

# 预览模式：只打印检测结果，不执行
npm run dev:patch:dry
```
...
| 改后端 `apps/backend/app` | 提示需完整打包（`--full`） | - |
...
| 变体 | 命令 | 体积 | 说明 |
|------|------|------|------|
| Full（默认） | `npm run dist:win` | ~250MB | 内置 Python 运行时，离线可用 |
| Light | `npm run dist:win:light` | ~50MB | 不内置 Python，首次启动自动下载 |
| Portable | `npm run dist:win:portable` | ~50MB | zip 免安装，解压即用 |

Light/Portable 通过 `prepare:runtime --no-embed` 跳过 Python 嵌入和 `.venv` 压缩，体积减少 80%。首次启动时后端检测到没有 `.venv` 会自动用绑定的 `uv` 创建。
...
- AIASys Desktop 安装包应通过 NSIS 脚本自动启用 Windows 长路径支持（`LongPathsEnabled`），并在需要时自动申请管理员权限；Agent 不应依赖用户手动处理路径长度或 UAC 弹窗。
- 为便于 Agent 在无显示/远程桌面环境下调试，Windows 桌面端打包时应预设以下非交互入口：
  - 支持 `/S` 静默安装，且安装程序自身能自动提升管理员权限。
