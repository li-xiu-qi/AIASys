...
```
config.json                          llm_config.json (运行时)
┌─────────────────────┐              ┌──────────────────────────────┐
│ llm:                │              │ providers: [                 │
...
| 层级 | 文件 | 作用 |
|------|------|------|
| 源配置 | `apps/backend/config.json` | 开发者维护，git 跟踪（含真实 key） |
| 运行时存储 | `data/workspaces/{user_id}/global_workspace/.aiasys/llm_config.json` | 系统读写，key 加密存储 |

## 关键规则

1. **`config.json` 不在 rsync 同步范围外** — 如果通过 rsync 同步代码，需要单独传输 `config.json`
2. **启动时自动同步** — 后端启动时调用 `sync_config_json_to_user()`，仅在运行时存储为空时才同步，不会覆盖已有配置
3. **同步后必须重启后端** — 修改 `config.json` 后需要重启 uvicorn 才能生效
...
```bash
# 在目标机器上
ls apps/backend/config.json
```

如果不存在，从源机器复制（**不要提交到 git**）：

```bash
scp apps/backend/config.json <用户>@<目标IP>:~/projects/AIASys/apps/backend/config.json
```
...
```bash
# 读取 key 前缀（不暴露完整 key）
grep api_key apps/backend/config.json
```
...
config = json.load(open("apps/backend/config.json"))
...
```bash
# 检查启动日志
grep "config.json 同步完成" logs/  # 或看 uvicorn 输出
# 期望看到: config.json 同步完成: N providers, M models
...
```
app.services.agent.mixins.session - ERROR - LLM 动态配置为空，请检查 config.json 和启动日志
```
...
| 原因 | 解决 |
|------|------|
| `config.json` 不存在 | 从源机器复制 |
| `config.json` 中 `llm.providers` 为空或格式错误 | 检查 JSON 格式 |
...
> 以上为项目当前使用的配置，实际以 `config.json` 为准。

## 故障排查速查

| 症状 | 检查点 |
|------|--------|
| 后端启动正常但 Agent 调用返回 400 | 检查 `config.json` 的 `api_key` 是否有效 |
...
| 文件 | 说明 |
|------|------|
| `apps/backend/config.json` | LLM 源配置（**不提交 git**） |
