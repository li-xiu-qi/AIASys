...
📋 **从模板创建工作区。** 系统内置多种工作区模板，覆盖数据分析、文献综述、项目开发等常见场景。一键创建即可预置好文件结构、环境变量和初始配置。也可以把当前工作区保存为模板，下次遇到同类任务直接复用，不需要每次都从零搭建环境。
...
⚙️ **配置模型。** 支持三种 LLM 接口协议：OpenAI Chat Completions、OpenAI Responses 和 Anthropic Messages。市面上绝大多数模型提供商都兼容这三种协议之一（kimi、DeepSeek、Qwen、GPT、Claude、Gemini 等），填好 API Key 和 Base URL 就能接入。前端有配置面板统一管理，支持多提供商同时启用、按工作区切换默认模型。
