...
import { gotoWorkspaceSession } from "./workspace-helpers";
import {
  createSession,
  createWorkspace,
  deleteSession,
...
test.describe("Workspace create flow browser regression", () => {
  test("empty session shows the workspace startup panel instead of legacy execution-space entry", async ({
    ...
    try {
      await gotoWorkspaceSession(page, sessionId);
  ...
  test("creating a new task activates a workspace route and keeps the new branch usable", async ({
    ...
    try {
      await gotoWorkspaceSession(page, sourceSessionId);
      await page.getByTestId("sidebar-new-task-expanded").click();
  ...
  test("new workspace conversations expose workspace config assets immediately", async ({
    ...
    try {
      await page.goto(
        `/workspace?workspace_id=${workspaceId}&session_id=${currentConversationId}`,
        {
